/**
 * Stitch-generated placeholder watch images, used while the backend's
 * `Asset` rows are empty in dev.
 *
 * TODO(media): delete this file once seeded items have real Cloudinary
 * URLs in `Asset.url`. The catalog/product-image components already
 * prefer real `assets[]` when present.
 *
 * Each entry maps a watch model "key" to a hosted image URL. Keys are
 * generated from the seed data — see `nest-backend/prisma/seed.ts`.
 */

export const watchPlaceholders: Record<string, string> = {
  "tudor-black-bay-58":
...
/**
 * Pick a deterministic placeholder for a watch by its model slug, falling
 * back to the brand initial placeholder rendered by `ProductImage`.
 */
export function placeholderFor(slug: string | undefined): string | undefined {
  if (!slug) return undefined;
  return watchPlaceholders[slug];
}

/** Best-effort slug from a watch's display name. */
export function slugifyWatch(brand: string, name: string): string {
  return `${brand}-${name}`
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/(^-|-$)/g, "");
